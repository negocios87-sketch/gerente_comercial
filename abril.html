...
function setTab(tab) {
  const tabs   = ['abril','forecast','historico'];
  const panels = ['panel-abril','panel-forecast','panel-historico'];
...
function renderHistorico(data) {
  ...
  allSquads.forEach(squad => {
    ...
    allDays.forEach((dia, i) => {
      ...
      const todayStr = new Date().toISOString().slice(0,10);
      const isPast   = dia < todayStr;
      const isToday  = dia === todayStr;
      const diaLabel = dia + (isToday ? '<span class="fc-today-badge">HOJE</span>' : '');

      tbody += `<tr class="row-group" data-fc-gid="${gid}" onclick="toggleFcDay('${gid}')" style="cursor:pointer;${isToday?'background:#FFFBEB !important;border-left:3px solid var(--gold);':''}">
        <td style="padding-left:14px;font-weight:600;color:var(--navy)">${diaLabel}</td>
        <td class="col-fc fc-media-val" style="background:rgba(13,122,62,.05)">${R(row.media_prevista||0)}</td>
        <td class="col-fc" style="color:var(--green);font-weight:600">${R(row.ganho||0)}</td>
        <td class="col-fc" style="color:var(--red)">${R(row.perdido||0)}</td>
        <td class="col-fc fc-gray">${R(row.remanejado||0)}</td>
        <td class="col-fc fc-total-bold">${R((row.ganho||0)+(row.perdido||0)+(row.remanejado||0))}</td>
        <td class="col-fc">${row.pct_atingimento != null ? pctBadgeFc(row.pct_atingimento) : '—'}</td>
      </tr>`;

      // Closer rows
      Object.entries(row.closers || {}).forEach(([ cname, cv ], ci) => {
        tbody += `<tr class="row-member" data-fc-mid="${gid}" style="display:none">
          ...
          <td class="col-fc fc-media-val" style="font-size:11px;background:rgba(13,122,62,.03)">${R(cv.media_prevista||0)}</td>
          <td class="col-fc" style="font-size:11px;color:var(--green)">${R(cv.ganho||0)}</td>
          <td class="col-fc" style="font-size:11px;color:var(--red)">${R(cv.perdido||0)}</td>
          <td class="col-fc fc-gray" style="font-size:11px">${R(cv.remanejado||0)}</td>
          <td class="col-fc" style="font-size:11px">${R((cv.ganho||0)+(cv.perdido||0)+(cv.remanejado||0))}</td>
          <td class="col-fc" style="font-size:11px">—</td>
        </tr>`;
      });
    });
    ...
    const totP = allDays.reduce((a,d) => { const s=datas.find(dd=>(snapshots[dd]||{})[squad]?.[d]); return a+(s?(snapshots[s][squad][d].perdido||0):0); }, 0);
    const totR = allDays.reduce((a,d) => { const s=datas.find(dd=>(snapshots[dd]||{})[squad]?.[d]); return a+(s?(snapshots[s][squad][d].remanejado||0):0); }, 0);
    ...
    tbody += `<tr class="row-total">
      ...
      <td style="color:var(--red)">${R(totP)}</td>
      <td style="color:var(--text-muted)">${R(totR)}</td>
      <td style="font-weight:700;color:var(--navy)">${R(totG+totP+totR)}</td>
        ...
        <div class="table-scroll">
          <table>
            <thead><tr>
              ...
              <th style="color:#6EE7B7">Ganho</th>
              <th style="color:#FCA5A5">Perdido</th>
              <th style="color:#9CA3AF">Remanejado</th>
