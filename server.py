...
function setTab(tab) {
  document.querySelectorAll('.tab').forEach((t,i) => t.classList.toggle('active', ['abril','forecast'][i]===tab));
  document.querySelectorAll('.panel').forEach((p,i) => p.classList.toggle('active', ['panel-abril','panel-forecast'][i]===`panel-${tab}`));
...
function toggleFcCloser(gid) {
  const grp = document.querySelector(`[data-fc-cgid="${gid}"]`);
  const members = document.querySelectorAll(`[data-fc-cmid="${gid}"]`);
...
function renderForecast(data) {
  ...
  Object.entries(squads).forEach(([squad, sq]) => {
    ...
    rows.forEach((row, i) => {
      ...
      // Day group row
      tbody += `<tr class="row-group" data-fc-gid="${gid}" onclick="toggleFcDay('${gid}')" style="cursor:pointer">
        <td style="padding-left:14px;font-weight:600;color:var(--navy)">${row.dia}</td>
        <td class="col-fc">${R(row.p20)}</td>
        <td class="col-fc">${R(row.p50)}</td>
        <td class="col-fc">${R(row.p70)}</td>
        <td class="col-fc val-real">${R(row.media)}</td>
        <td class="col-fc">${R(row.em_aberto)}</td>
        <td class="col-fc" style="color:${row.realizado>0?'var(--green)':'var(--text-muted)'}">${R(row.realizado)}</td>
        <td class="col-fc" style="color:${row.perda>0?'var(--red)':'var(--text-muted)'}">${R(row.perda)}</td>
        <td class="col-fc">${R(row.total_previsto)}</td>
        <td class="col-fc">${pctBadgeFc(row.atingimento)}</td>
      </tr>`;
    ...
    tbody += `<tr class="row-total">
      ...
      <td class="val-real">${R(tot.media)}</td>
      <td>${R(tot.em_aberto)}</td>
        ...
        <div class="table-scroll">
          <table>
            <thead><tr>
              ...
              <th>Média Prev.</th><th>Em Aberto</th>
              <th>Realizado</th><th>Perda</th>
              <th>Total Prev.</th><th>% Ating.</th>
            </tr></thead>
            <tbody>${tbody}</tbody>
          </table>
        </div>
